...
describe('HypeSDK', () => {
	...
	describe('initialization', () => {
		it('should initialize with default values', () => {
			...
			expect(sdk.programId.toString()).toBe(
				'HYPExvaQRQHrkCNc1DAHJoByUeBqFvkJyhtpFdacLcdH',
			)
		})

		it('should throw error when using methods before initRoot', async () => {
			const publicKey = new PublicKey(
				'G1XZNbpX2GoPHPEV71JrNvRicRnTYzjmrJfXQdbUnhoZ',
			)
			await expect(sdk.getUserTokens(publicKey)).rejects.toThrow(
				'Init root first!',
			)
		})
	})
